/* Copyright (c) 2012-2021 Richard Rodger and other contributors, MIT License */
'use strict'
...
function Entity(canon, seneca) {
  // NOTE: creates an intermediate prototype to hide internals.
  ...
  // Setting custom properties by calling this function deep clones
  // the properties onto the entity.
  this.custom$ = function (props) {
    if (
      null != props &&
...
Entity.prototype.make$ = function (...args) {
  const self = this
  ...
  self.log$ && self.log$('make', entity.canon$({ string: true }), entity)
...
Entity.prototype.save$ = async function (data, done) {
  ...
  let done$ = null == done ? undefined : this.done$ ? this.done$(done) : done
...
Entity.prototype.load$ = function (query, done) {
  ...
  var q = normalize_query(query, self)
