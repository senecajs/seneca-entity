...
function Entity(canon, seneca) {
  ...
  self.log$ = function() {
    ...
    this.private$.get_instance().log.apply(this, arguments)
  }

  var private$ = (self.private$ = function() {})

  private$.get_instance = function() { return seneca }
...
Entity.prototype.make$ = function() {
  ...
  if (args[0] && args[0].seneca) {
    ...
    self.private$.get_instance = function() { return seneca }
  }
 ...
 */
Entity.prototype.save$ = function(data, done) {
  var self = this
  ...
  si.act(self.private$.entargs({ cmd: 'save' }), done)
  return self
}
...
Entity.prototype.load$ = function(query, done) {
  ...
  done = (_.isFunction(query) ? query : done) || _.noop
  ...
  si.act(self.private$.entargs({ qent: qent, q: q, cmd: 'load' }), done)
...
Entity.prototype.list$ = function(query, done) {
  ...
  si.act(self.private$.entargs({ qent: qent, q: q, cmd: 'list' }), done || _.noop)
...
Entity.prototype.remove$ = function(query, done) {
  ...
  si.act(
    self.private$.entargs({ qent: self, q: q, cmd: 'remove' }),
    done || _.noop
  )
...
Entity.prototype.canon$ = function(opt) {
  ...
  if (opt) {
    if (opt.isa) {
      ...
      return _.every(['zone', 'base', 'name'], function(label) {
        return (
          isa[label] === canon[label] ||
          (isa[label] == null && canon[label] == null)
        )
      })
    } else if (opt.parse) {
  ...
  return _.isUndefined(opt) || opt.string || opt.string$
    ? [
        ...
        _.isUndefined(canon.name) ? '-' : canon.name
      ].join('/')
    : opt.array
...
Entity.prototype.clone$ = function() {
  var self = this
  return self.make$(this.private$.get_instance().util.deepextend({}, self.data$()))
}
...
function resolve_id_query(qin, ent) {
  ...
  if (
    (_.isUndefined(qin) || _.isNull(qin) || _.isFunction(qin)) &&
    ent.id != null
  ) {
    q = { id: ent.id }
