...
Entity.prototype.save$ = function (data, done) {
  ...
  si.act(self.private$.entargs({ cmd: 'save', q: q }), done)
  return self
}
...
Entity.prototype.load$ = function (query, done) {
  ...
  done = 'function' === typeof(query) ? query : done || noop
...
Entity.prototype.list$ = function (query, done) {
  ...
  si.act(
    self.private$.entargs({ qent: qent, q: q, cmd: 'list' }),
    done || noop
  )
...
function resolve_id_query(qin, ent) {
  ...
  if (
    (null == qin || 'function' === typeof(qin)) &&
    ent.id != null
  ) {
    q = { id: ent.id }
  } else if ('string' === typeof(qin) || _.isNumber(qin)) {
    q = qin === '' ? null : { id: qin }
...
function parsecanon(str) {
  ...
  if (_.isObject(str) && 'function' !== typeof(str)) return str
...
function make_toString(canon_str, hidden_fields_spec, opts) {
  ...
  var hidden_fields = [].concat(
    Array.isArray(hidden_fields_spec) ? hidden_fields_spec : []
  )
